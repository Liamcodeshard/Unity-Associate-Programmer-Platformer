    ...
    void OnCollisionEnter(Collision other)
    {
        if (canKill == true)
        {
            if (getHitScript.hurt == false)
            {
                ...
                {
                    patrolEnemyScript = other.gameObject.GetComponent<PatrolEnemy>();
                    patrolEnemyScript.enemyStats.speed = 0;
                    enemyObject = other.gameObject;
                    Kill();
                    StartCoroutine("DestroyPatrol");
